	...
	def after_insert(self):
		if self.invoice_type=="POS Invoice":
			if self.loyalty_points > 0:
				frappe.db.set_value("POS Invoice", self.invoice, "loyalty_point_earn", self.loyalty_points)
				conversion_rate = frappe.db.get_value("Loyalty Program", self.loyalty_program, "conversion_factor")
				frappe.db.set_value("POS Invoice", self.invoice, "loyalty_point_earn_amount", self.loyalty_points *conversion_rate )
